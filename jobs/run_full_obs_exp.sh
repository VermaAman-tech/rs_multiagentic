...
if [ "$VLLM_SHARED_READY" -eq 0 ] || [ "$VLLM_VRA_READY" -eq 0 ]; then
  echo "Error: vLLM failed to start within 10 minutes. Check logs/vllm_shared_obs.log and logs/vllm_vra_obs.log"
  exit 1
fi
echo "Both vLLM servers are ready. Starting full observability pipeline run..."
