...
from tools.vision import object_detection, segmentation, description, text_to_bbox, region_attribute, counting, ocr, change_detection, draw_box, add_text
from tools.gis import area_boundary, pois_layer, compute_distance, display_map, bbox_from_geotiff, display_geotiff, display_on_geotiff
from tools.spectral import add_index_layer, compute_index_change, show_index_layer
from tools.general import google_search, plot

app = FastAPI(title="MAGRF Tool Server", version="1.0")

@app.get("/health")
def health():
    return {"status": "ok", "n_tools": 31}

# --- NEW TOOLS (GoogleSearch, Plot, DisplayOnGeotiff) ---
...
@app.post("/tools/GoogleSearch")
def run_google_search(req: GoogleSearchInput):
    return google_search.google_search(query=req.query, k=req.k)

@app.post("/tools/Plot")
def run_plot(req: PlotInput):
    return plot.plot(command=req.command)

@app.post("/tools/DisplayOnGeotiff")
def run_display_on_geotiff(req: DisplayOnGeotiffInput):
    return display_on_geotiff.display_on_geotiff(geotiff_path=req.geotiff_path, gpkg_path=req.gpkg_path, layer_name=req.layer_name)
...
def run_temporal_stack(req: TemporalStackLoaderInput):
    res = temporal_stack_loader.temporal_stack_loader(
        aoi_bbox=req.aoi_bbox, date_range=req.date_range, sensor=req.sensor, max_cloud_pct=req.max_cloud_pct
    )
...
def run_road_damage(req: RoadDamageScorerInput):
    res = road_damage_scorer.road_damage_scorer(
        gpkg_path=req.gpkg_path, road_layer_name=req.road_layer_name, damage_raster_layer=req.damage_raster_layer, buffer_meters=req.buffer_meters
    )
...
def run_evacuation(req: EvacuationRoutePlannerInput):
    res = evacuation_route_planner.evacuation_route_planner(
        graph_path=req.graph_path, origins=req.origins, destinations=req.destinations, blocked_segments=req.blocked_segments
    )
...
def run_calculator(req: CalculatorInput):
    try:
        ...
        allowed = {"__builtins__": {}, "math": math, "abs": abs, "round": round,
                   "min": min, "max": max, "sum": sum, "pow": pow, "int": int, "float": float}
        result = float(eval(compile(req.expression, "<calc>", "eval"), allowed))
    except Exception:
        result = 0.0
